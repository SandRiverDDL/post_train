...
- Qwen3-4B-Instruct-2507 正确 rollout 数据 `data/stage1/qwen3_4b_instruct_2507_math_correct_len4096/train.jsonl` 上，Qwen3-1.7B BF16 LoRA rank32 ASFT-topK 已完成。`checkpoint-50/100/150` 在 MATH500 为 `0.670/0.676/0.686`，在 GSM8K 为 `0.8226/0.8324/0.8158`；dev200 为 `0.665/0.660/0.660`，`checkpoint-187` 降到 `0.630`。ckpt 间差异接近噪声；综合 MATH500/GSM8K 默认用 `checkpoint-100`，只看 MATH500 可用 `checkpoint-150`，不建议只按 dev200 选择 `checkpoint-50`。
- 同数据 SFT 对照已完成，配置为 `configs/stage1/qwen3_1p7b_qwen3_4b_instruct2507_math_correct_len4096_sft.yaml`，输出为 `outputs/stage1_qwen3_1p7b_bf16_lora_qwen3_4b_instruct2507_math_correct_len4096_sft_ddp3_b2_acc6_bs36`。Qwen3-1.7B BF16 LoRA rank32，三卡 DDP；`batch_size=3`、`gradient_accumulation_steps=4` 在首个 backward OOM，最终使用 `batch_size=2`、`gradient_accumulation_steps=6`，有效 batch `36`，共 `172` optimizer steps，训练用时约 `863s`，train loss `1.3681`。产物包含 `checkpoint-50/100/150/172`。
- 同数据 SFT 已完成三卡并行 dev200 checkpoint 选择，输出为 `outputs/stage1_qwen3_1p7b_bf16_lora_qwen3_4b_instruct2507_math_correct_len4096_sft_ddp3_b2_acc6_bs36/dev_eval_parallel/`。`checkpoint-50/100/150/172` 的 dev200 acc 分别为 `0.615/0.620/0.625/0.615`，boxed rate 为 `0.775/0.795/0.765/0.750`，平均输出 tokens 为 `502.6/499.9/528.0/532.7`。当前按 dev200 选择 `checkpoint-150`，但差距小于 `0.03`，属于高噪声区。
- 当前 SFT/DFT loss 中 gold-token NLL 已从 full-vocab `log_softmax(...).gather(...)` 改为等价的 `logsumexp(logits) - gold_logit`，避免为普通 SFT 额外 materialize 完整 log-prob tensor；小张量 contract check 已验证 loss 和梯度与标准 CE 对齐。`.venv` 当前缺少 `pytest`，未运行 `tests/test_sft.py`。
- 后续若基于 ASFT `checkpoint-100` 做 student rollout，默认目标是 OPD/行为诊断，不是 teacher SFT 数据扩增；应复用 Qwen3 chat-template eval 口径，`prompt_style=justrl_math`、`system_prompt=""`、`assistant_prefill="<think>\n\n</think>\n\n"`、`max_lora_rank=32`。采样建议比 Instruct teacher rollout 更保守：先用 `temperature=0.6~0.7`、`top_p=0.8`、`top_k=20`、`max_new_tokens=2048` 做 100-200 题 probe；如截断率和重复率低，再扩到 `max_new_tokens=3072`。不建议直接沿用 Instruct teacher 的探索口径 `temperature=0.8/top_p=1.0/top_k=32/max_new_tokens=3584`。
- Qwen3-1.7B SFT `checkpoint-150` student rollout 的 Lightning-OPD 数据已完成 teacher top16 forward：`data/lightning_opd/qwen3_1p7b_sft150_math_nontrunc_top16_instruct2507/train.jsonl` 共 `6329` 条，shape error `0`。标准 OPD 训练只用 sampled-token teacher logprob，`top16` 仅作分析；为避免每次 DDP rank 重建 1.6GB JSONL，已新增 `scripts/convert_lightning_opd_dataset.py`，并生成可直接 `load_from_disk` 的 `hf_dataset_opd_top1/`，大小约 `221M`，后续优先使用 `configs/lightning_opd/train_qwen3_1p7b_sft150_math_nontrunc_hfdataset_lr1e5.yaml`。
- Qwen3-1.7B SFT150 -> Qwen3-4B-Instruct-2507 teacher 的 Lightning-OPD 训练已完成：配置 `configs/lightning_opd/train_qwen3_1p7b_sft150_math_nontrunc_top16_lr1e5.yaml`，输出 `outputs/lightning_opd_qwen3_1p7b_sft150_math_nontrunc_top16_lr1e5`，起点为 SFT `checkpoint-150`，三卡 DDP，LoRA rank32，LR `1e-5`，有效 batch `36`，共 `176` optimizer steps，训练用时约 `884s`，train loss `-0.8043`。训练日志无 OOM/Traceback，仅退出时有 NCCL destroy warning。
- 该 Lightning-OPD 训练的 token-level 诊断显示 teacher avg logprob 仍低于 student avg logprob：teacher 均值约 `-0.365`、student 均值约 `-0.216`、gap 均值约 `-0.149`。这不代表链路错误：数据是 student rollout，不是 teacher 原始轨迹；SFT/OPD student 对自身 trajectory 会更尖锐；同时训练集约 `1062/6329=16.8%` 样本 source_correct=false。OPD 训练只是在 sampled tokens 上对齐 teacher logprob，不能假设 teacher 对所有 student token 都更高置信。
- 该 Lightning-OPD 已完成 dev200 多长度复核。`max_new_tokens=2048` 口径 best 为 `checkpoint-10`：acc `0.635`，boxed `0.720`；但 2048 截断很重，不能单独作为结论。`max_new_tokens=4096/max_seq_length=5096` 下，`checkpoint-10` 仍最好：acc `0.675`，boxed `0.775`，cap4096 `47/200=23.5%`；`checkpoint-50` 降到 acc `0.455`、boxed `0.500`、cap4096 `105/200=52.5%`；`checkpoint-150` 为 acc `0.590`、boxed `0.655`、cap4096 `73/200=36.5%`。进一步把 `checkpoint-10/50/150` 放到 `max_new_tokens=8192/max_seq_length=9216`，acc 分别为 `0.680/0.575/0.640`，cap8192 分别为 `44/200`、`88/200`、`62/200`。结论：放宽长度只能小幅缓解，不能救回后期 ckpt；当前 sampled-token OPD 主要问题是训练后快速变长、no-box 和重复推理，而不是单纯评测 cap 太短。
- 该 Lightning-OPD 数据还暴露 EOS 监督缺失：Qwen3 tokenizer 的 EOS 为 `<|im_end|>`，但 `data/lightning_opd/qwen3_1p7b_sft150_math_nontrunc_top16_instruct2507/train.jsonl` 中 `response_mask` 最后 supervised token 没有 EOS，response 文本也不含 `<|im_end|>`。普通 SFT tokenization 已会追加 EOS；当前 Lightning-OPD path 没有追加。该问题可能加重“正确 boxed 后继续生成”的不停止现象，但不是唯一原因，sampled-token gap 偏负、16.8% source incorrect、LR `1e-5` 和 top1 离线信号仍是主要风险。
...
- 已重跑 JustRL-Nemotron chat-template 50 条 probe：`data/rollout/math_sft/justrl_nemotron_chat_probe50_len8192/raw/raw_samples.merged.jsonl`。`<think>` 从裸 prompt 的 `8/50` 提升到 `50/50`，`</think>` 为 `42/50`；该旧 rollout 脚本记录的 `output_tokens` 实际是 whitespace word count，均值 `2805.5`、`p95=5071.5`、max `6099` 不能和 tokenizer token 口径直接比较。用 JustRL-Nemotron tokenizer 对 response 重算后，均值约 `4954.7` tokens、`p50=4333`、`p90=8088.3`、`p95=8109`、max `8127`；正确样本均值约 `4373.4`，错误样本均值约 `5903.2`。结论是 chat template 修复了起始格式，但输出很长，不能直接 raw SFT；后续 teacher 长度比较必须同题、同 tokenizer 口径。
...
- Thinking Machines 风格 OPD baseline 已新增 MATH level prompt 数据入口：`scripts/prepare_verl_opd_data.py --mode math-levels --levels ... --sample-size ... --max-prompt-tokens 256`。当前小测试默认改为 Level 4、500 题：L4 原始 `1690` 条，Qwen3 chat-template prompt 超过 256 tokens 的样本 `82` 条，过滤后剩 `1608` 条；已生成 `../verl/data/opd_math/hendrycks_l4_prompt256_500_seed42/train.parquet`，抽样 prompt token mean `84.5`、p95 `154.05`、max `251`。三卡 wrapper 为 `runs/opd/run_tm_qwen3_math_l34_1k.sh`，默认 student=SFT150 两卡、teacher=Qwen3-4B-Instruct-2507 一卡、`loss_mode=k1`、`use_policy_gradient=True`、`use_task_rewards=False`、`n=1`、`MAX_PROMPT=256`、`MAX_RESPONSE_LENGTH=2048`。本轮暂不改 `../verl` 做 special-token mask；若后续出现提前 EOS、`</think>` 偏置或格式坍缩，再最小 patch response mask。
...
- 当前 JustRL-Nemotron 与 Qwen3-4B NT 的旧 probe 不是同题集合，且旧 `rollout_math_sft.py` 长度字段不是 tokenizer tokens；不能直接用 JustRL `2805.5` 对比 Qwen3-4B NT `3537.3`。按 tokenizer 口径，JustRL-Nemotron chat probe50 反而更长；Qwen3-4B NT probe60 为均值 `3537.3`、`p95=7596.4`、max `8192`，但二者必须重新同题 probe 后再选 teacher。
- 当前 Qwen3-4B-Instruct-2507 教师 probe 已在固定 MATH train 60 题上完成，产物为 `data/rollout/teacher_sft/qwen3_4b_instruct_2507_math_probe60_len4096/`。口径为 Qwen3 chat template、`max_new_tokens=4096`、`max_model_len=8192`、三卡 shard、`temperature=0.8/top_p=1.0/top_k=32`。结果：correct `52/60=86.7%`，boxed/parse `54/60=90.0%`，截断 `7/60=11.7%`，response token 均值 `1344.6`、p50 `663`、p75 `2021.25`、p90/p95/max 均到 `4096`；rendered prompt token 均值 `108.2`、p95 `247.8`、max `755`。若 full MATH rollout 目标是训练样本 `prompt+response<=4096`，当前建议用 `max_new_tokens=3584`、`max_model_len=5120`，生成后再过滤 `prompt+response<=4096`；官方 sampling 默认应优先用 `temperature=0.7/top_p=0.8/top_k=20`，而不是 probe 中较发散的探索参数。固定 60 题上 `3584` 预计 `8/60=13.3%` 会达到/超过 cap，且 cap 后 `prompt+response` 最大约 `3828`。
...
7. Lightning OPD 当前 topK=1 sampled-token 信号偏负，且 Qwen3-1.7B SFT150 -> Instruct2507 这轮在 dev200 上出现快速变长、no-box 与重复推理；下一步若继续 OPD，应先修复 EOS 监督、降低 LR/步数、过滤错误与重复样本，或测试 topK support / SFT anchor，而不是直接增加 epoch。
