...
def _parameter_count(model, *, trainable_only: bool = False, name_substring: str | None = None) -> int:
    total = 0
...
def collect_adapter_diagnostics(model, cfg) -> dict[str, Any]:
    ...
    requested_is_adapter_dir = _looks_like_adapter_dir(requested_path)
    ...
    diagnostics: dict[str, Any] = {
        ...
        "trainable_param_count": _parameter_count(model, trainable_only=True),
        "total_param_count": _parameter_count(model),
        "lora_param_count": _parameter_count(model, name_substring="lora_"),
        "trainable_lora_param_count": _parameter_count(model, trainable_only=True, name_substring="lora_"),
...
def train_grpo(cfg, reward_cfg) -> Path:
    ...
    reward_funcs = build_reward_functions(reward_cfg, max_completion_length=cfg.max_completion_length)
